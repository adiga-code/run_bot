...
**Правило:**
```python
def route_to_program(level: int) -> "new" | "manual":
    if level in (1, 2, 3):
...
Для уровней 4 и 5 новая логика недельного планирования НЕ запускается:
- `WeekPlan` не создаётся
- Прогрессия объёма не считается
- Чек-ин и маркинг работают (для телеметрии)
- Тренер сам отправляет тренировки через админку (как в текущей системе)
- Уведомление при онбординге: «Тебя ведёт тренер вручную»

В `assign_level` опционально: если уровень получился 4, проверить, можно ли его понизить до 3 (если близко к границе). Иначе оставить 4 и роутить на manual.
...
**Прогрессия:**
```
Если неделя успешная (все 6 условий из 3.9.1):
  next_week_target = round_int(actual_running_minutes × 1.10)

Если неделя неуспешная:
  next_week_target = current_weekly_target  (удержание плана)

Разгрузочная неделя (триггер из 3.9.5):
...
После разгрузочной (следующая неделя):
  next_week_target = round_int(peak_volume × 1.10)
